  ...
  passport.use(
    new GoogleStrategy(
      ...
      async function (accessToken, refreshToken, profile, done) {
        ...
        // check if user already exists in our db with the given the email
        const existingUser = await Admin.findOne({ officialEmail: email }).populate('companies lawyers') || await Company.findOne({ officialEmail: email }) || await Lawyer.findOne({ officialEmail: email });
