...
export const assignJob = async (req, res) => {
    ...
    try {     
        ...
        if(job.assignedTo.includes(lawyerId)){
            return res.status(400).json({ error: 'Lawyer already assigned to this job'})
        }
...
export const applyForJob = async(req, res) => {
    ...
    try{
        ...
        if(job.assignedTo.length !== 0 || job.assignedTo !== null || job.assignedTo.length !== undefined ){
            return res.status(400).json({ error: 'A Lawyer already assigned to this job'})
        }
        if(lawyer.verified == true){
